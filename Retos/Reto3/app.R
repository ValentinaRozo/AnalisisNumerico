...
options(digits= 22)
## Lectura de archivos
nacional = read_excel("/Users/valentinarozobernal/Desktop/Reto3Poblacion/Poblaciones.xlsx", 
                      sheet = "Nacional ")
bogota = read_excel("/Users/valentinarozobernal/Desktop/Reto3Poblacion/Poblaciones.xlsx", 
                    sheet = "Bogota")
antioquia = read_excel("/Users/valentinarozobernal/Desktop/Reto3Poblacion/Poblaciones.xlsx", 
                       sheet = "Antioquia")
...
if (interactive()) {
  ...
  ui <- fluidPage(
    ...
    sidebarLayout(
      sidebarPanel(
        ...
        radioButtons("M",
                     "Municipio:",
                     choices=list("Nacional"=1,"Bogot??"=2,"Antioquia"=3),
                     selected=1),
        radioButtons("G",
                     "Género:",
  ...
  server <- function(input, output, session) {
    observe({
      ...
      output$Total<-renderText({
        ...
        paste("La población para el A?o ",input$Tf," de ", palabra," es: ", res)
      })
    })
  }
  ...
  Graficar<-function(t0,tf,p0, pf, p5, modelo, carga, poblacion){
    ...
    par(mfrow=c(1,3))
    plot(x,y,xlab="tiempo (Nº A?os)",ylab="poblacion",main=palabra,type="l", col="black")
    lines( x, p, col = "red")
    lines( x, media, col = "green")
    legend("topleft", legend = c("Media", "Exactos", "Simulación"), col= 3:1, pch=1)
    
    plot(x, err, col = "red", type = "l", xlab="tiempo (Nº A?os)", ylab="Error", main = "Error relativo")
    plot(x, errTr, col = "red", type = "l", xlab="tiempo (Nº A?os)", ylab="Error", main = "Error de truncamiento")
    ...
    # Entropia de Kolmogrov-Sina? Malthus
    ...
    # Entropia de Kolmogrov-Sina? Logistica
